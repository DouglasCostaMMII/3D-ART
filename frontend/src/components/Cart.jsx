...
import { contact, cart as cartContent, products } from '../config/content'
...
function formatVariation(variation) {
  ...
  return Object.entries(variation)
    .filter(([, v]) => v)
    .map(([k, v]) => `${products.variationLabels[k] || k}: ${v}`)
    .join(', ')
}

function buildWhatsAppMessage(items, totalPrice) {
  const { greeting, itemsHeader, totalPrefix, paymentQuestion } = cartContent.whatsappMessage
  const lines = [greeting, '', itemsHeader]
  ...
  lines.push('')
  lines.push(`${totalPrefix} ${formatPrice(totalPrice)}*`)
  lines.push('')
  lines.push(paymentQuestion)
...
export default function Cart() {
  ...
  function handleWhatsApp() {
    ...
    window.open(`https://wa.me/${contact.whatsapp}?text=${encoded}`, '_blank', 'noopener,noreferrer')
  }
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-white">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-gray-900">{cartContent.title}</h2>
            {totalItems > 0 && (
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-16 px-4">
              ...
              <p className="text-gray-500 font-medium">{cartContent.empty}</p>
              <p className="text-gray-400 text-sm mt-1">{cartContent.emptySubtitle}</p>
              <button
                onClick={closeCart}
                className="mt-4 bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-indigo-700 transition-colors text-sm"
              >
                {cartContent.viewProducts}
              </button>
            </div>
          ) : (
            items.map((item) => {
        ...
        {items.length > 0 && (
          <div className="border-t border-gray-100 px-5 py-5 bg-white space-y-4">
            ...
            <div className="flex items-center justify-between">
              <span className="text-gray-600 font-medium">{cartContent.total}</span>
